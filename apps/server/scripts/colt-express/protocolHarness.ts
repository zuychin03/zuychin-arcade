...
import type { JoinRoomResponse, ColtPrivateState, ColtPublicState, RoomPublicState } from '@zuychin-arcade/types';
...
process.env.JWT_SECRET ??= 'colt_express-isolated-test-secret-32-bytes';

export interface Client {
  auth: JoinRoomResponse;
  socket: Socket;
  room: RoomPublicState | null;
  game: ColtPublicState | null;
  mine: ColtPrivateState | null;
...
export async function harness() {
  ...
  async function create(name = 'Astra Host', password?: string) {
    const response = await request('/rooms/create', { gameId: 'colt_express', displayName: name, ...(password === undefined ? {} : { password }) });
  ...
  async function connect(auth: JoinRoomResponse): Promise<Client> {
    ...
    socket.on('colt:action_accepted', (value) => { client.acks.push(value); });
...
export function assertPrivacy(client: Client): void {
  ...
  for (const player of game.players) {
    for (const field of ['hand', 'deck', 'discard', 'reserved', 'lootByBandit']) assert.ok(!Object.hasOwn(player, field));
    if (game.status !== 'game_over') {
      assert.equal(player.lootValue, null);
      assert.equal(player.finalScore, null);
    }
    if (game.phase === 'team_selection' || game.phase === 'team_setup' || game.phase === 'character_selection') assert.deepEqual(player.positions, []);
    if (!player.characterChosen) {
      assert.equal(player.character, null);
      assert.deepEqual(player.characters, []);
    }
  }
  if (game.phase === 'character_selection') assert.deepEqual(mine.hand, []);
  for (const field of ['rng', 'roundDeck', 'eventQueue']) assert.ok(!Object.hasOwn(game, field));
  for (const loot of Object.values(game.lootBySpace).flat()) {
    if (loot.type === 'purse') assert.equal(loot.value, null);
  }
  for (let i = 0; i < game.program.length; i += 1) {
    const card = game.program[i]!;
    if (!card.faceUp && i > game.executionIndex && game.status !== 'game_over') {
      assert.equal(card.action, null);
      assert.equal(card.ownerBandit, null);
    }
  }
}
