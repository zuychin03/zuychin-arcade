...
import type { CitadelsActionKind } from '@zuychin-arcade/types';
import { getSocket } from '../../hooks/useSocket';
import { useGameStore } from '../../store/useGameStore';


type Pending = { action: CitadelsActionKind; expectedRevision: number; acceptedRevision: number | null; label: string };

export function useCitadelsActions() {
  const token = useGameStore((state) => state.token);
  const citadelsPublic = useGameStore((state) => state.citadelsPublic);
  const citadelsPrivate = useGameStore((state) => state.citadelsPrivate);
  const storeSyncing = useGameStore((state) => state.citadelsSyncing);
  ...
  const requestState = useCallback(() => {
    ...
    useGameStore.getState().setCitadelsSyncing(true);
    setSynced(false);
  ...
  useEffect(() => {
    ...
    const paired = () => {
      ...
      if (latest.citadelsSyncing || latest.citadelsPublic?.revision !== current.public || latest.citadelsPrivate?.revision !== current.private) return;
    ...
    const receivedPublic = (state: { gameId?: string; roomCode?: string; revision?: number }) => {
      ...
      const latest = useGameStore.getState();
      if (latest.token !== token || state.gameId !== 'citadels' || state.roomCode !== latest.roomCode || typeof state.revision !== 'number' || !Number.isSafeInteger(state.revision) || state.revision < Math.max(0, latest.citadelsPublic?.revision ?? 0, revisions.current.public ?? 0, revisions.current.private ?? 0)) return;
    ...
    const receivedPrivate = (state: { gameId?: string; roomCode?: string; playerId?: string; revision?: number }) => {
      if (!ownsSession() || !state || typeof state !== 'object' || Array.isArray(state)) return;
      const latest = useGameStore.getState();
      if (latest.token !== token || state.gameId !== 'citadels' || state.roomCode !== latest.roomCode || state.playerId !== latest.playerId || typeof state.revision !== 'number' || !Number.isSafeInteger(state.revision) || state.revision < Math.max(0, latest.citadelsPrivate?.revision ?? 0, revisions.current.public ?? 0, revisions.current.private ?? 0)) return;
      revisions.current.private = state.revision;
      if (revisions.current.public !== state.revision) setSynced(false);
      paired();
    };
    const accepted = (ack: { action: CitadelsActionKind; revision: number }) => {
    ...
    socket.on('citadels:action_accepted', accepted);
    ...
    return () => {
      ...
      socket.off('citadels:action_accepted', accepted);
  ...
  useEffect(() => {
    const revision = citadelsPublic?.revision;
    const command = pendingRef.current;
    if (storeSyncing || !connected || revision === undefined || citadelsPrivate?.revision !== revision
    ...
    setSynced(true);
    if (command) finish(`${command.label} accepted.`);
  }, [connected, storeSyncing, citadelsPublic?.revision, citadelsPrivate?.revision, finish]);
  ...
  const ready = connected && synced && !storeSyncing && citadelsPublic?.revision === citadelsPrivate?.revision;
  const send = (action: CitadelsActionKind, payload: Record<string, unknown>, label: string, expectedRevision: number): boolean => {
    const latest = useGameStore.getState();
    const socket = getSocket();
    const owner = binding.current;
    if (!owner?.active || owner.socket !== socket || owner.token !== token || !ready || latest.token !== token || latest.citadelsPrivate?.playerId !== latest.playerId || latest.citadelsPrivate?.roomCode !== latest.roomCode || latest.citadelsPublic?.roomCode !== latest.roomCode || latest.citadelsSyncing || !socket?.connected || pendingRef.current || latest.citadelsPublic?.revision !== expectedRevision
      || latest.citadelsPrivate?.revision !== expectedRevision || revisions.current.public !== expectedRevision || revisions.current.private !== expectedRevision) return false;
    ...
    else socket.emit(`citadels:${action.replaceAll('_', '-')}`, { ...payload, expectedRevision });
    return true;
  };
