...
export function useKrakenLeave(dismissOverlay: () => boolean) {
  const token = useGameStore(state => state.token);
  const status = useGameStore(state => state.krakenPublic?.status);
  ...
  const requestLeave = () => {
    ...
    const captured = useGameStore.getState().krakenPublic;
    showDialog(captured && captured.status !== 'game_over' ? 'Forfeit this game?' : 'Leave the table?',
      captured && captured.status !== 'game_over'
        ? 'Your seat forfeits and cannot win. The voyage may continue. This is not a feeding sacrifice.'
        ...
        { text: 'LEAVE', style: 'destructive', onPress: () => {
          ...
          if (!mounted.current || latest.token !== token || latest.krakenPublic?.status !== captured?.status) return;
  ...
  useWebBackGuard('/feed-the-kraken/game', () => handleBack.current(), Boolean(token));
  useEffect(() => {
