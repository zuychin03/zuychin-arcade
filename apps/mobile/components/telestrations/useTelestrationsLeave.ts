...
export function useTelestrationsLeave(dismissOverlay: () => boolean) {
  const token = useGameStore(state => state.token);
  const status = useGameStore(state => state.telestrationsPublic?.phase);
  ...
  const requestLeave = () => {
    ...
    const captured = useGameStore.getState().telestrationsPublic;
    showDialog(captured && captured.phase !== 'game_over' ? 'Forfeit this game?' : 'Leave the table?',
      captured && captured.phase !== 'game_over'
        ? 'Your seat will forfeit. The unfinished round is cancelled; with fewer than four players, the game ends without a winner.'
        ...
        { text: 'LEAVE', style: 'destructive', onPress: () => {
          ...
          if (!mounted.current || latest.token !== token || latest.telestrationsPublic?.phase !== captured?.phase) return;
  ...
  useWebBackGuard('/telestrations/game', () => handleBack.current(), Boolean(token));
  useEffect(() => {
