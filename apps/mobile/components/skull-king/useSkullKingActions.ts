...
import type { SkullKingActionKind } from '@zuychin-arcade/types';
import { getSocket } from '../../hooks/useSocket';
import { useGameStore } from '../../store/useGameStore';


type Pending = { action: SkullKingActionKind; expectedRevision: number; acceptedRevision: number | null; label: string };

export function useSkullKingActions() {
  const token = useGameStore((state) => state.token);
  const skullKingPublic = useGameStore((state) => state.skullKingPublic);
  const skullKingPrivate = useGameStore((state) => state.skullKingPrivate);
  const storeSyncing = useGameStore((state) => state.skullKingSyncing);
  ...
  const requestState = useCallback(() => {
    ...
    useGameStore.getState().setSkullKingSyncing(true);
    setSynced(false);
  ...
  useEffect(() => {
    ...
    const paired = () => {
      ...
      if (latest.skullKingSyncing || latest.skullKingPublic?.revision !== current.public || latest.skullKingPrivate?.revision !== current.private) return;
    ...
    const receivedPublic = (state: { gameId?: string; roomCode?: string; revision?: number }) => {
      ...
      const latest = useGameStore.getState();
      if (latest.token !== token || state.gameId !== 'skull_king' || state.roomCode !== latest.roomCode || typeof state.revision !== 'number' || !Number.isSafeInteger(state.revision) || state.revision < Math.max(0, latest.skullKingPublic?.revision ?? 0, revisions.current.public ?? 0, revisions.current.private ?? 0)) return;
    ...
    const receivedPrivate = (state: { gameId?: string; roomCode?: string; playerId?: string; revision?: number }) => {
      if (!ownsSession() || !state || typeof state !== 'object' || Array.isArray(state)) return;
      const latest = useGameStore.getState();
      if (latest.token !== token || state.gameId !== 'skull_king' || state.roomCode !== latest.roomCode || state.playerId !== latest.playerId || typeof state.revision !== 'number' || !Number.isSafeInteger(state.revision) || state.revision < Math.max(0, latest.skullKingPrivate?.revision ?? 0, revisions.current.public ?? 0, revisions.current.private ?? 0)) return;
      revisions.current.private = state.revision;
      if (revisions.current.public !== state.revision) setSynced(false);
      paired();
    };
    const accepted = (ack: { action: SkullKingActionKind; revision: number }) => {
    ...
    socket.on('skull_king:action_accepted', accepted);
    ...
    return () => {
      ...
      socket.off('skull_king:action_accepted', accepted);
  ...
  useEffect(() => {
    const revision = skullKingPublic?.revision;
    const command = pendingRef.current;
    if (storeSyncing || !connected || revision === undefined || skullKingPrivate?.revision !== revision
    ...
    setSynced(true);
    if (command) finish(`${command.label} accepted.`);
  }, [connected, storeSyncing, skullKingPublic?.revision, skullKingPrivate?.revision, finish]);
  ...
  const ready = connected && synced && !storeSyncing && skullKingPublic?.revision === skullKingPrivate?.revision;
  const send = (action: SkullKingActionKind, payload: Record<string, unknown>, label: string, expectedRevision: number): boolean => {
    const latest = useGameStore.getState();
    const socket = getSocket();
    const owner = binding.current;
    if (!owner?.active || owner.socket !== socket || owner.token !== token || !ready || latest.token !== token || latest.skullKingPrivate?.playerId !== latest.playerId || latest.skullKingPrivate?.roomCode !== latest.roomCode || latest.skullKingPublic?.roomCode !== latest.roomCode || latest.skullKingSyncing || !socket?.connected || pendingRef.current || latest.skullKingPublic?.revision !== expectedRevision
      || latest.skullKingPrivate?.revision !== expectedRevision || revisions.current.public !== expectedRevision || revisions.current.private !== expectedRevision) return false;
    ...
    else socket.emit(`skull_king:${action}`, { ...payload, expectedRevision });
    return true;
  };
