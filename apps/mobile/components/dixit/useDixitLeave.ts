...
export function useDixitLeave(dismissOverlay: () => boolean) {
  const token = useGameStore(state => state.token);
  const status = useGameStore(state => state.dixitPublic?.status);
  ...
  const requestLeave = () => {
    ...
    const captured = useGameStore.getState().dixitPublic;
    showDialog(captured?.status === 'playing' ? 'Forfeit this game?' : 'Leave the table?',
      captured?.status === 'playing'
        ? 'Your seat will forfeit. The unfinished round is cancelled; with fewer than three players, the game ends without a winner.'
        ...
        { text: 'LEAVE', style: 'destructive', onPress: () => {
          ...
          if (!mounted.current || latest.token !== token || latest.dixitPublic?.status !== captured?.status) return;
          dismissPrompt(); void leave();
        } },
      ]);
    prompt.current = useDialogStore.getState().dialog;
  };
  handleBack.current = () => { if (!dismissOverlay()) requestLeave(); };
  useWebBackGuard('/dixit-odyssey/game', () => handleBack.current(), Boolean(token));
  useEffect(() => {
