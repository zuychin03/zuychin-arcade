...
import type { SaboteurActionKind } from '@zuychin-arcade/types';
import { getSocket } from '../../hooks/useSocket';
import { useGameStore } from '../../store/useGameStore';


type Pending = { action: SaboteurActionKind; expectedRevision: number; acceptedRevision: number | null; label: string };

export function useSaboteurActions() {
  const token = useGameStore((state) => state.token);
  const publicState = useGameStore((state) => state.publicState);
  const privateState = useGameStore((state) => state.privateState);
  const storeSyncing = useGameStore((state) => state.saboteurSyncing);
  ...
  const requestState = useCallback(() => {
    ...
    useGameStore.getState().setSaboteurSyncing(true);
    setSynced(false);
  ...
  const armTimeout = useCallback(() => {
    ...
    timeout.current = setTimeout(() => {
      ...
      finish('No confirmation received. Refreshing the mine before you try again.');
      requestState();
    }, 12_000);
  ...
  useEffect(() => {
    ...
    const paired = () => {
      ...
      if (latest.saboteurSyncing || latest.publicState?.revision !== current.public || latest.privateState?.revision !== current.private) return;
    ...
    const receivedPublic = (state: { gameId?: string; roomCode?: string; revision?: number }) => {
      ...
      const latest = useGameStore.getState();
      if (latest.token !== token || state.gameId !== 'saboteur' || state.roomCode !== latest.roomCode || typeof state.revision !== 'number' || !Number.isSafeInteger(state.revision) || state.revision < Math.max(1, latest.publicState?.revision ?? 1, revisions.current.public ?? 1, revisions.current.private ?? 1)) return;
    ...
    const receivedPrivate = (state: { gameId?: string; roomCode?: string; playerId?: string; revision?: number }) => {
      if (!ownsSession() || !state || typeof state !== 'object' || Array.isArray(state)) return;
      const latest = useGameStore.getState();
      if (latest.token !== token || state.gameId !== 'saboteur' || state.roomCode !== latest.roomCode || state.playerId !== latest.playerId || typeof state.revision !== 'number' || !Number.isSafeInteger(state.revision) || state.revision < Math.max(1, latest.privateState?.revision ?? 1, revisions.current.public ?? 1, revisions.current.private ?? 1)) return;
      revisions.current.private = state.revision;
      if (revisions.current.public !== state.revision) setSynced(false);
      paired();
    };
    const accepted = (ack: { action: SaboteurActionKind; revision: number }) => {
    ...
    const rejected = (payload: { reason?: unknown } | null | undefined) => {
      ...
      finish(`Action not accepted: ${reason}. Refreshing the mine.`);
      requestState();
    };
    const disconnected = () => {
      if (!ownsSession()) return;
      ...
      setConnected(false);
      setSynced(false);
      setMessage('Connection lost. Your controls will return after the mine reconnects.');
    };
    const reconnected = () => {
      if (!ownsSession()) return;
      finish();
      setConnected(true);
      setMessage('Connected. Refreshing your private hand and the mine.');
      requestState();
    };
    socket.on('game_state', receivedPublic);
    socket.on('private_state', receivedPrivate);
    socket.on('saboteur:action_accepted', accepted);
    ...
    return () => {
      ...
      socket.off('saboteur:action_accepted', accepted);
  ...
  useEffect(() => {
    const revision = publicState?.revision;
    const command = pendingRef.current;
    if (storeSyncing || !connected || revision === undefined || privateState?.revision !== revision
    ...
    setSynced(true);
    if (command) finish(`${command.label} confirmed.`);
  }, [connected, storeSyncing, publicState?.revision, privateState?.revision, finish]);
  ...
  const ready = connected && synced && !storeSyncing && publicState?.revision === privateState?.revision;
  const send = (action: SaboteurActionKind, payload: Record<string, unknown>, label: string, expectedRevision: number): boolean => {
    const latest = useGameStore.getState();
    const socket = getSocket();
    const owner = binding.current;
    if (!owner?.active || owner.socket !== socket || owner.token !== token || !ready || latest.token !== token || latest.privateState?.playerId !== latest.playerId || latest.publicState?.roomCode !== latest.roomCode || latest.saboteurSyncing || !socket?.connected || pendingRef.current || latest.publicState?.revision !== expectedRevision
      || latest.privateState?.revision !== expectedRevision || revisions.current.public !== expectedRevision || revisions.current.private !== expectedRevision) return false;
    ...
    else socket.emit(action, { ...payload, expectedRevision });
    return true;
  };
