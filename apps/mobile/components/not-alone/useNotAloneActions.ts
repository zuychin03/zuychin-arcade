...
import type { NotAloneActionKind } from '@zuychin-arcade/types';
import { getSocket } from '../../hooks/useSocket';
import { useGameStore } from '../../store/useGameStore';


type Pending = { action: NotAloneActionKind; expectedRevision: number; acceptedRevision: number | null; label: string };

export function useNotAloneActions() {
  const token = useGameStore((state) => state.token);
  const notAlonePublic = useGameStore((state) => state.notAlonePublic);
  const notAlonePrivate = useGameStore((state) => state.notAlonePrivate);
  const storeSyncing = useGameStore((state) => state.notAloneSyncing);
  ...
  const requestState = useCallback(() => {
    ...
    useGameStore.getState().setNotAloneSyncing(true);
    setSynced(false);
  ...
  useEffect(() => {
    ...
    const paired = () => {
      ...
      if (latest.notAloneSyncing || latest.notAlonePublic?.revision !== current.public || latest.notAlonePrivate?.revision !== current.private) return;
    ...
    const receivedPublic = (state: { gameId?: string; roomCode?: string; viewerPlayerId?: string; revision?: number }) => {
      if (!ownsSession() || !state || typeof state !== 'object' || Array.isArray(state)) return;
      const latest = useGameStore.getState();
      if (state.viewerPlayerId !== latest.playerId) return;
      if (latest.token !== token || state.gameId !== 'not_alone' || state.roomCode !== latest.roomCode || typeof state.revision !== 'number' || !Number.isSafeInteger(state.revision) || state.revision < Math.max(0, latest.notAlonePublic?.revision ?? 0, revisions.current.public ?? 0, revisions.current.private ?? 0)) return;
    ...
    const receivedPrivate = (state: { gameId?: string; roomCode?: string; playerId?: string; revision?: number }) => {
      if (!ownsSession() || !state || typeof state !== 'object' || Array.isArray(state)) return;
      const latest = useGameStore.getState();
      if (latest.token !== token || state.gameId !== 'not_alone' || state.roomCode !== latest.roomCode || state.playerId !== latest.playerId || typeof state.revision !== 'number' || !Number.isSafeInteger(state.revision) || state.revision < Math.max(0, latest.notAlonePrivate?.revision ?? 0, revisions.current.public ?? 0, revisions.current.private ?? 0)) return;
      revisions.current.private = state.revision;
      if (revisions.current.public !== state.revision) setSynced(false);
      paired();
    };
    const accepted = (ack: { action: NotAloneActionKind; revision: number }) => {
    ...
    socket.on('notalone:action_accepted', accepted);
    ...
    return () => {
      ...
      socket.off('notalone:action_accepted', accepted);
  ...
  useEffect(() => {
    const revision = notAlonePublic?.revision;
    const command = pendingRef.current;
    if (storeSyncing || !connected || revision === undefined || notAlonePrivate?.revision !== revision
    ...
    setSynced(true);
    if (command) finish(`${command.label} accepted.`);
  }, [connected, storeSyncing, notAlonePublic?.revision, notAlonePrivate?.revision, finish]);
  ...
  const ready = connected && synced && !storeSyncing && notAlonePublic?.revision === notAlonePrivate?.revision;
  const send = (action: NotAloneActionKind, payload: Record<string, unknown>, label: string, expectedRevision: number): boolean => {
    const latest = useGameStore.getState();
    const socket = getSocket();
    const owner = binding.current;
    if (latest.notAlonePublic?.viewerPlayerId !== latest.playerId) return false;
    if (!owner?.active || owner.socket !== socket || owner.token !== token || !ready || latest.token !== token || latest.notAlonePrivate?.playerId !== latest.playerId || latest.notAlonePrivate?.roomCode !== latest.roomCode || latest.notAlonePublic?.roomCode !== latest.roomCode || latest.notAloneSyncing || !socket?.connected || pendingRef.current || latest.notAlonePublic?.revision !== expectedRevision
      || latest.notAlonePrivate?.revision !== expectedRevision || revisions.current.public !== expectedRevision || revisions.current.private !== expectedRevision) return false;
    const command = { action, expectedRevision, acceptedRevision: null, label };
    pendingRef.current = command;
    setPending(command);
    setMessage(`${label}…`);
    armTimeout();
    if (action === 'start') socket.emit('start_game', payload);
    else socket.emit(`notalone:${action.replaceAll('_', '-')}`, { ...payload, expectedRevision });
    return true;
  };
