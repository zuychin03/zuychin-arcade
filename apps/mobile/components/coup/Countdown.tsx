...
export function Countdown({ deadline, color = COUP.crimson }: { deadline: number | null; color?: string }) {
  ...
  return (
    <View
      ...
      accessibilityLabel="Response time remaining"
      accessibilityValue={{ min: 0, max: Math.round(RESPONSE_TIMEOUT_MS / 1000), now: secs, text: `${secs} seconds` }}
      style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}
    >
      <View style={{ flex: 1, height: 6, borderRadius: 3, backgroundColor: COUP.border, overflow: 'hidden' }}>
