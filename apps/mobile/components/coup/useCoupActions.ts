...
import type { CoupActionKind } from '@zuychin-arcade/types';
import { getSocket } from '../../hooks/useSocket';
import { useGameStore } from '../../store/useGameStore';


type Pending = { action: CoupActionKind; expectedRevision: number; acceptedRevision: number | null; label: string };

export function useCoupActions() {
  const token = useGameStore((state) => state.token);
  const coupPublic = useGameStore((state) => state.coupPublic);
  const coupPrivate = useGameStore((state) => state.coupPrivate);
  const storeSyncing = useGameStore((state) => state.coupSyncing);
  ...
  const requestState = useCallback(() => {
    ...
    useGameStore.getState().setCoupSyncing(true);
    setSynced(false);
  ...
  const armTimeout = useCallback(() => {
    ...
    timeout.current = setTimeout(() => {
      ...
      finish('No confirmation received. Refreshing the court before you try again.');
      requestState();
    }, 12_000);
  ...
  useEffect(() => {
    ...
    const paired = () => {
      ...
      if (latest.coupSyncing || latest.coupPublic?.revision !== current.public || latest.coupPrivate?.revision !== current.private) return;
    ...
    const receivedPublic = (state: { gameId?: string; roomCode?: string; revision?: number }) => {
      ...
      const latest = useGameStore.getState();
      if (latest.token !== token || state.gameId !== 'coup' || state.roomCode !== latest.roomCode || typeof state.revision !== 'number' || !Number.isSafeInteger(state.revision) || state.revision < Math.max(0, latest.coupPublic?.revision ?? 0, revisions.current.public ?? 0, revisions.current.private ?? 0)) return;
    ...
    const receivedPrivate = (state: { gameId?: string; roomCode?: string; playerId?: string; revision?: number }) => {
      if (!ownsSession() || !state || typeof state !== 'object' || Array.isArray(state)) return;
      const latest = useGameStore.getState();
      if (latest.token !== token || state.gameId !== 'coup' || state.roomCode !== latest.roomCode || state.playerId !== latest.playerId || typeof state.revision !== 'number' || !Number.isSafeInteger(state.revision) || state.revision < Math.max(0, latest.coupPrivate?.revision ?? 0, revisions.current.public ?? 0, revisions.current.private ?? 0)) return;
      revisions.current.private = state.revision;
      if (revisions.current.public !== state.revision) setSynced(false);
      paired();
    };
    const accepted = (ack: { action: CoupActionKind; revision: number }) => {
    ...
    const rejected = (payload: { reason?: unknown } | null | undefined) => {
      ...
      finish(`Action not accepted: ${reason}. Refreshing the court.`);
      requestState();
    };
    const disconnected = () => {
      if (!ownsSession()) return;
      ...
      setConnected(false);
      setSynced(false);
      setMessage('Connection lost. Your controls will return after the court reconnects.');
    };
    const reconnected = () => {
      if (!ownsSession()) return;
      finish();
      setConnected(true);
      setMessage('Connected. Refreshing your private influences and the court.');
      requestState();
    };
    socket.on('game_state', receivedPublic);
    socket.on('private_state', receivedPrivate);
    socket.on('coup:action_accepted', accepted);
    ...
    return () => {
      ...
      socket.off('coup:action_accepted', accepted);
  ...
  useEffect(() => {
    const revision = coupPublic?.revision;
    const command = pendingRef.current;
    if (storeSyncing || !connected || revision === undefined || coupPrivate?.revision !== revision
    ...
    setSynced(true);
    if (command) finish(`${command.label} accepted.`);
  }, [connected, storeSyncing, coupPublic?.revision, coupPrivate?.revision, finish]);
  ...
  const ready = connected && synced && !storeSyncing && coupPublic?.revision === coupPrivate?.revision;
  const send = (action: CoupActionKind, payload: Record<string, unknown>, label: string, expectedRevision: number): boolean => {
    const latest = useGameStore.getState();
    const socket = getSocket();
    const owner = binding.current;
    if (!owner?.active || owner.socket !== socket || owner.token !== token || !ready || latest.token !== token || latest.coupPrivate?.playerId !== latest.playerId || latest.coupPrivate?.roomCode !== latest.roomCode || latest.coupPublic?.roomCode !== latest.roomCode || latest.coupSyncing || !socket?.connected || pendingRef.current || latest.coupPublic?.revision !== expectedRevision
      || latest.coupPrivate?.revision !== expectedRevision || revisions.current.public !== expectedRevision || revisions.current.private !== expectedRevision) return false;
    ...
    else socket.emit(`coup:${action}`, { ...payload, expectedRevision });
    return true;
  };
