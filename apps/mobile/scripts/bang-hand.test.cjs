...
const source = read('components/bang/Hand.tsx');
const route = read('app/bang/game.tsx');
...
const cards = count => Array.from({ length: count }, (_, index) => ({ id: `card-${index}`, name: 'bang', rank: `${index + 1}`, suit: 'hearts' }));
...
function harness({ width = 375, fontScale = 1, platform = 'web', reduceMotion = false } = {}) {
  ...
  const modules = {
    ...
    '../../constants/theme': { BANG: { surface: 'surface', panel: 'panel', border: 'border', gold: 'gold', muted: 'muted', sand: 'sand' } },
  ...
  return { ...exports, moves, actions, render(props) {
    cursor = 0; pending = [];
    const tree = exports.BangHand({ renderCard: (card, onFocus, fluid) => jsx('Card', { card, onFocus, fluid, selected: card.id === 'card-1', disabled: card.id === 'card-2', status: `status-${card.id}`, onPress: () => actions.push(card.id) }, card.id), ...props });
...
function measure(instance, props, viewport = 311) {
  ...
  find(tree, 'bang-hand-cards').props.onLayout({ nativeEvent: { layout: { width: viewport } } });
  tree = instance.render(props);
  const rail = find(tree, 'bang-hand-rail');
  if (!rail) return tree;
...
test('measured full-card widths fit narrow viewports, peek when space permits and bound native enlargement', () => {
  ...
  for (const viewport of [1, 180, 256, 272, 288, 311, 600, 1200]) for (const scale of [1, 1.5, 2, 3]) for (const web of [true, false]) {
    ...
    assert.equal(bangHandWidth(viewport, scale, web), available < 280 ? available : Math.min(360 * (web ? 1 : scale), available - 20));
  }
  for (const invalid of [NaN, Infinity, 0, -10]) assert.equal(bangHandWidth(invalid, NaN, false), 272);
});

test('browse and focus geometry clamps endpoints and ignores invalid measurements', () => {
  const { bangHandBrowseOffset: browse, bangHandFocusOffset: focus, bangHandPosition: position } = harness();
...
test('empty and one/two-card hands remain simple; measured desktop grids collapse only at the text-aware threshold', () => {
  for (const count of [0, 1, 2, 3, 6]) {
    const instance = harness(), props = { cards: cards(count) };
    ...
    if (count === 1 || count === 2) {
      assert.equal(find(tree, 'bang-hand-grid').props.style.flexWrap, 'wrap');
  ...
  for (const textScale of [1, 1.5, 2]) for (const viewport of [256, 311, 599, 600, 900, 1200]) {
    ...
    const tree = measure(instance, { cards: cards(6), textScale }, viewport);
    assert.equal(Boolean(find(tree, 'bang-hand-rail')), viewport < 600 * textScale);
    assert.equal(Boolean(find(tree, 'bang-hand-grid')), viewport >= 600 * textScale);
    assert.equal(handCards(tree).length, 6);
  }
  assert(find(measure(harness({ width: 1280, fontScale: 2, platform: 'ios' }), { cards: cards(6) }, 900), 'bang-hand-rail'));
});
...
test('browse controls have fixed 48px hit areas, boundary state and no game-selection side effects', () => {
  for (const platform of ['web', 'ios', 'android']) for (const reduceMotion of [false, true]) {
    const instance = harness({ platform, reduceMotion }), props = { cards: cards(6) };
    ...
    assert.equal(find(tree, 'bang-hand-position').props.children.join(''), 'Card 6 of 6 · swipe or browse');
    ...
    assert.equal(handCards(tree)[2].props.status, 'status-card-2'); assert(handCards(tree).every(card => card.props.fluid));
  }
});

test('keyboard focus and swipe reveal cards only within the rail without animation, submission or page navigation', () => {
  for (const platform of ['web', 'ios', 'android']) {
    const instance = harness({ platform }), props = { cards: cards(6) };
    let tree = instance.render(props);
    ...
    find(tree, 'bang-hand-rail').props.onScroll({ nativeEvent: { contentOffset: { x: 590 } } });
    rendered[2].props.onFocus(); assert.equal(instance.moves.at(-1).x, 0, 'Already-visible card after swipe does not scroll');
    tree = instance.render(props); assert.equal(find(tree, 'bang-hand-position').props.children.join(''), 'Card 3 of 6 · swipe or browse');
    for (const move of instance.moves) { assert.equal(move.animated, false); assert.equal(move.y, undefined); }
    assert.deepEqual(instance.actions, []);
    rendered[0].props.onPress(); assert.deepEqual(instance.actions, ['card-0']);
  }
...
test('unrelated revisions, status and selection renders preserve rail position; hand identity resets it', () => {
  const instance = harness(), props = { cards: cards(6) };
  let tree = measure(instance, props); buttons(tree)[1].props.onPress();
  instance.moves.length = 0;
  tree = instance.render({ ...props, cards: props.cards.map(card => ({ ...card, status: 'changed' })), textScale: 1.1, renderCard: (card, onFocus, fluid) => ({ type: 'Card', props: { card, onFocus, fluid, disabled: true, selected: false } }) });
  assert.deepEqual(instance.moves, []); assert.equal(find(tree, 'bang-hand-position').props.children.join(''), 'Card 2 of 6 · swipe or browse');
  tree = instance.render({ ...props, cards: [props.cards[1], props.cards[0], ...props.cards.slice(2)] });
  assert.equal(instance.moves.length, 1); assert.equal(instance.moves[0].x, 0); assert.equal(instance.moves[0].animated, false);
  instance.moves.length = 0;
  instance.render({ cards: props.cards.slice(1) }); assert.equal(instance.moves.length, 1); assert.equal(instance.moves[0].x, 0);
});
...
test('removing the first card reveals the actual requested survivor without new child layout events', () => {
  for (const platform of ['web', 'ios', 'android']) for (const viewport of [256, 311, 480]) {
    ...
    measure(instance, { cards: original }, viewport);
    const props = { cards: original.slice(1) };
    let tree = instance.render(props);
    const rail = find(tree, 'bang-hand-rail');
    const wrappers = nodes(rail).filter(node => node.type === 'View' && typeof node.props.style?.width === 'number');
    const width = wrappers[0].props.style.width, gap = rail.props.contentContainerStyle.gap;
    rail.props.onContentSizeChange(props.cards.length * width + (props.cards.length - 1) * gap + 8);
    ...
    for (const index of [0, 3, 5, 0]) {
      ...
      tree = instance.render(props);
      if (index === 0) assert.equal(find(tree, 'bang-hand-position').props.children.join(''), 'Card 1 of 6 · swipe or browse');
    }
    assert.deepEqual(instance.actions, []);
    for (const move of instance.moves) { assert.equal(move.animated, false); assert.equal(move.y, undefined); }
    handCards(tree)[0].props.onPress(); assert.deepEqual(instance.actions, [original[1].id]);
  }
});

test('same-count replacement and reorder use current ordered geometry without survivor layout callbacks', () => {
  for (const mutate of [list => [list[1], list[0], ...list.slice(2)], list => [...list.slice(1), { ...list[0], id: 'new-equipment' }]]) {
    const instance = harness(), original = cards(6);
    measure(instance, { cards: original });
    const props = { cards: mutate(original) };
    let tree = instance.render(props); tree = instance.render(props);
    instance.moves.length = 0;
    assert.equal(find(tree, 'bang-hand-position').props.children.join(''), 'Card 1 of 6 · swipe or browse');
...
test('rail to grid to rail resize resets the new scroll owner and keeps label and boundaries consistent', () => {
  ...
  assert.equal(buttons(tree)[1].props.disabled, true);
  tree = measure(instance, props, 800); assert(find(tree, 'bang-hand-grid'));
  instance.moves.length = 0;
...
test('private-hand integration preserves card status, disabled state, selection order and every original action branch', () => {
  const ast = ts.createSourceFile('game.tsx', route, ts.ScriptTarget.Latest, true, ts.ScriptKind.TSX);
  const found = [];
  function visit(node) { if (ts.isJsxSelfClosingElement(node) && node.tagName.getText(ast) === 'BangHand') found.push(node); ts.forEachChild(node, visit); }
  ...
  assert.equal(expr('cards'), 'priv.hand'); assert.equal(expr('textScale'), 'textScale');
  const jsx = (type, props, key) => ({ type, props, key });
  for (const mode of ['select', 'sid', 'discard', 'respond', 'rescue']) for (const busy of [true, false]) {
    const calls = [], card = cards(1)[0], onFocus = () => {};
    const priv = { hand: [card], playOptions: [{ cardId: card.id }], responseCardIds: [card.id], rescueBeerCardIds: [card.id], canDiscard: mode === 'discard', canRespond: mode === 'respond', canRescue: mode === 'rescue' };
    const context = { exports: {}, require: () => ({ jsx, jsxs: jsx }), BangCardView: 'Card', sidMode: mode === 'sid', priv, busy, selectedId: card.id, picks: [card.id], required: 3, cardStatus: () => 'Allowed', name: () => 'BANG!', toggle: (...args) => args, setPicks: value => calls.push(['picks', value]), send: (...args) => calls.push(args), setSelectedId: value => calls.push(['selected', value]), setTargetId: value => calls.push(['target', value]) };
    vm.runInNewContext(compile(`globalThis.renderCard = (${expr('renderCard')});`), context);
    const result = context.renderCard(card, onFocus, true);
    assert.equal(result.props.card, card); assert.equal(result.props.onFocus, onFocus); assert.equal(result.props.fluid, true);
    assert.equal(result.props.disabled, busy); assert.equal(result.props.selected, true); assert.equal(result.props.status, 'Allowed');
    assert.equal(result.props.selectionOrder, mode === 'discard' ? 1 : undefined);
    result.props.onPress();
    assert.equal(calls[0][0], { select: 'selected', sid: 'picks', discard: 'picks', respond: 'respond', rescue: 'rescue' }[mode]);
    if (mode === 'select') assert.deepEqual(calls, [['selected', card.id], ['target', null]]);
    if (mode === 'sid' || mode === 'discard') assert.equal(calls[0][1][2], mode === 'sid' ? 2 : 3);
    if (mode === 'respond' || mode === 'rescue') assert.equal(calls[0][1].cardId, card.id);
    priv.playOptions = []; priv.responseCardIds = []; priv.rescueBeerCardIds = []; priv.canDiscard = false; context.sidMode = false;
    assert.equal(context.renderCard(card).props.disabled, true);
  }
});
