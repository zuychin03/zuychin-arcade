...
function layout(route, reduced) {
  ...
  const modules = {
    ...
    '../../constants/theme': Object.fromEntries(['MINE', 'COUP', 'TOKYO', 'SKULL_KING', 'CITADELS', 'NOT_ALONE', 'BANG', 'LIBERTALIA', 'COLT'].map(name => [name, palette])),
...
for (const [route, title] of games) {
  test(`${route}: readable navigation title preserves full identity, back control and motion preferences`, () => {
    for (const reduced of [false, true]) {
      ...
      assert.equal(tree.props.screenOptions.headerTitleStyle.fontWeight, '800');
      assert.equal(tree.props.screenOptions.headerTitleStyle.fontSize, undefined);
